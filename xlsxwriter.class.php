...
class XLSXWriter
{
    ...
    public function writeSheetHeader($sheet_name, array $header_types, $reformatHeaders = false, $col_options = null)
    {
        ...
        if (!$suppress_row)
        {
            $header_row = $reformatHeaders ? $this->formatHeaders(array_keys($header_types)) : array_keys($header_types);
    ...
    private function formatHeaders(array $headers): array
    {
        foreach ($headers as $key => $header) {
            if($header != 'odbior_w_trybie_48h') {
                $headers[$key] = preg_replace('/[0-9]/', '', $header);
            }
        }
        return $headers;
    }
